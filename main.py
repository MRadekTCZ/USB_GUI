...
def MRB_ON():
    ...
    #recv_text.insert(tkinter.END, usb_port.read(12).decode('Ascii'))
def MRB_OFF():
    usb_port.write('AA22'.encode('Ascii'))
    #recv_text.insert(tkinter.END, usb_port.read(14).decode('Ascii'))
def SOFT_START():
...
def REFRESH_FREQ():
    ...
    actual_freq_received = usb_port.read(10).decode('Ascii')
...
end_freq_entry = tkinter.Entry(usb_gui_frame_Ramp)
end_freq_entry.grid(row=0, column=0, padx=10)
ramp_time_entry = tkinter.Entry(usb_gui_frame_Ramp)
ramp_time_entry.grid(row=1, column=0, padx=10)
btn_setramp = tkinter.Button(usb_gui_frame_Ramp, text='Set frequency', command=SET_FREQ)
